...
class HTTPStrategy {
    ...
    async connect(url, { fetch: fetcher, prepare, auth, ns, db, } = {}) {
        this.http = new SurrealHTTP_js_1.SurrealHTTP(url, { fetcher });
    ...
    async query(query) {
        if (arguments[1]) {
            throw new Error("The query function in the HTTP strategy does not support data as the second argument.");
        }
        await this.ready;
